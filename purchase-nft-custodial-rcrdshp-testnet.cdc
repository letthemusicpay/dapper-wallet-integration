...
transaction(listingResourceID: UInt64, storefrontAddress: Address, expectedPrice: UFix64) {
    ...
    prepare(custodialBuyer: AuthAccount, dapper: AuthAccount, buyer: AuthAccount) {
        ...
        // Get the collection from the custodialBuyer so the NFT can be deposited into it
        self.nftCollection = getAccount(custodialBuyer).borrow<&RCRDSHPNFT.Collection{NonFungibleToken.Receiver}>(
